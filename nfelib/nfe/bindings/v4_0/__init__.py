"""Auto-generated schema metadata.

This module is part of the generated binding package `nfelib.nfe.bindings.v4_0`.
The source XSD schema package details are tracked below.
"""

# Source schema package information (from CHANGELOG_SCHEMAS.md)
SCHEMA_VERSION: str = 'PL_010f'
SCHEMA_PACKAGE: str = 'Pacote de Liberação 010f (zip PL_010f_v1.04)'
NOTA_TECNICA: str = 'NT 2025.002 v1.50, NT 2026.007 v1.00'
PUBLISHED_AT: str = '2026-08-31'
SOURCE_URL: str = 'https://www.nfe.fazenda.gov.br/portal/exibirArquivo.aspx?conteudo=8ITFuBLltXs='
GENERATED_AT: str = '2026-09-25T20:29:10Z'

"""This file was generated by xsdata, v25.7, on 2026-09-25 17:17:39

Generator: DataclassGenerator
See: https://xsdata.readthedocs.io/
"""

from nfelib.nfe.bindings.v4_0.cons_reci_nfe_v4_00 import ConsReciNfe
from nfelib.nfe.bindings.v4_0.cons_sit_nfe_v4_00 import ConsSitNfe
from nfelib.nfe.bindings.v4_0.cons_stat_serv_v4_00 import ConsStatServ
from nfelib.nfe.bindings.v4_0.dfe_tipos_basicos_v1_00 import (
    TajusteCompet,
    Talczfmcbs,
    TalczfmcbsNfe,
    TalczfmcbsNfeTpAlczfmcbs,
    Tcibs,
    TcibsNfe,
    TcompraGov,
    TcompraGovReduzido,
    TcredPres,
    TcredPresIbszfm,
    TcredPresOper,
    TdevTrib,
    Tdif,
    TenteGov,
    TestornoCred,
    TibscbsmonoTot,
    Tibscbstot,
    TindDoacao,
    Tis,
    Tistot,
    Tmonofasia,
    ToperCompraGov,
    TpagRef,
    Tred,
    TtpCredPresIbszfm,
    TtransfCred,
    TtribBpe,
    TtribCompraGov,
    TtribCte,
    TtribNf3E,
    TtribNfag,
    TtribNfce,
    TtribNfcom,
    TtribNfe,
    TtribNfgas,
    TtribRegular,
)
from nfelib.nfe.bindings.v4_0.envi_nfe_v4_00 import EnviNfe
from nfelib.nfe.bindings.v4_0.inut_nfe_v4_00 import InutNfe
from nfelib.nfe.bindings.v4_0.leiaute_cons_sit_nfe_v4_00 import (
    TconsSitNfe,
    TconsSitNfeXServ,
    Tevento,
    TprocEvento,
    TprotNfe,
    TretCancNfe,
    TretConsSitNfe,
    TretEvento,
    TverConsSitNfe,
)
from nfelib.nfe.bindings.v4_0.leiaute_cons_stat_serv_v4_00 import (
    TconsStatServ,
    TconsStatServXServ,
    TretConsStatServ,
)
from nfelib.nfe.bindings.v4_0.leiaute_inut_nfe_v4_00 import (
    InfInutXServ,
    TinutNfe,
    TprocInutNfe,
    TretInutNfe,
)
from nfelib.nfe.bindings.v4_0.leiaute_nfe_v4_00 import (
    ArmaTpArma,
    CardTpIntegra,
    CofinsaliqCst,
    CofinsntCst,
    CofinsoutrCst,
    CofinsqtdeCst,
    CofinsstIndSomaCofinsst,
    DestIndIedest,
    DetPagIndPag,
    DiTpIntermedio,
    DiTpViaTransp,
    EmitCrt,
    GuiaTransitoTpGuia,
    Icms00Cst,
    Icms00ModBc,
    Icms02Cst,
    Icms10Cst,
    Icms10ModBc,
    Icms10ModBcst,
    Icms10MotDesIcmsst,
    Icms15Cst,
    Icms15MotRedAdRem,
    Icms20Cst,
    Icms20IndDeduzDeson,
    Icms20ModBc,
    Icms20MotDesIcms,
    Icms30Cst,
    Icms30IndDeduzDeson,
    Icms30ModBcst,
    Icms30MotDesIcms,
    Icms40Cst,
    Icms40IndDeduzDeson,
    Icms40MotDesIcms,
    Icms51Cst,
    Icms51ModBc,
    Icms53Cst,
    Icms60Cst,
    Icms61Cst,
    Icms70Cst,
    Icms70IndDeduzDeson,
    Icms70ModBc,
    Icms70ModBcst,
    Icms70MotDesIcms,
    Icms70MotDesIcmsst,
    Icms90Cst,
    Icms90IndDeduzDeson,
    Icms90ModBc,
    Icms90ModBcst,
    Icms90MotDesIcms,
    Icms90MotDesIcmsst,
    IcmspartCst,
    IcmspartIndDeduzDeson,
    IcmspartModBc,
    IcmspartModBcst,
    IcmspartMotDesIcms,
    Icmssn101Csosn,
    Icmssn102Csosn,
    Icmssn201Csosn,
    Icmssn201ModBcst,
    Icmssn202Csosn,
    Icmssn202ModBcst,
    Icmssn500Csosn,
    Icmssn900Csosn,
    Icmssn900ModBc,
    Icmssn900ModBcst,
    IcmsstCst,
    IcmsufdestPIcmsinter,
    IdeIdDest,
    IdeIndFinal,
    IdeIndIntermed,
    IdeIndPres,
    IdeTpEmis,
    IdeTpImp,
    IdeTpNf,
    IpintCst,
    IpitribCst,
    IssqnIndIncentivo,
    IssqnIndIss,
    IssqntotCRegTrib,
    OrigCombIndImport,
    PisaliqCst,
    PisntCst,
    PisoutrCst,
    PisqtdeCst,
    PisstIndSomaPisst,
    ProcRefIndProc,
    ProcRefTpAto,
    ProdIndBemMovelUsado,
    ProdIndEscala,
    ProdIndTot,
    RefEcfMod,
    RefNfMod,
    RefNfpMod,
    TconsReciNfe,
    Tendereco,
    TenderEmi,
    TenderEmiCPais,
    TenderEmiXPais,
    TenviNfe,
    TenviNfeIndSinc,
    TfinNfe,
    TinfRespTec,
    Tipi,
    Tlocal,
    Tnfe,
    TnfeProc,
    Torig,
    TprocEmi,
    TranspModFrete,
    TretConsReciNfe,
    TretEnviNfe,
    TtpNfcredito,
    TtpNfdebito,
    Tveiculo,
    VeicProdCondVeic,
    VeicProdTpOp,
    VeicProdTpRest,
    VeicProdVin,
)
from nfelib.nfe.bindings.v4_0.nfe_v4_00 import Nfe
from nfelib.nfe.bindings.v4_0.proc_inut_nfe_v4_00 import ProcInutNfe
from nfelib.nfe.bindings.v4_0.proc_nfe_v4_00 import NfeProc
from nfelib.nfe.bindings.v4_0.ret_cons_reci_nfe_v4_00 import RetConsReciNfe
from nfelib.nfe.bindings.v4_0.ret_cons_sit_nfe_v4_00 import RetConsSitNfe
from nfelib.nfe.bindings.v4_0.ret_cons_stat_serv_v4_00 import RetConsStatServ
from nfelib.nfe.bindings.v4_0.ret_envi_nfe_v4_00 import RetEnviNfe
from nfelib.nfe.bindings.v4_0.ret_inut_nfe_v4_00 import RetInutNfe
from nfelib.nfe.bindings.v4_0.tipos_basico_v4_00 import (
    Tamb,
    TcodUfIbge,
    TcorgaoIbge,
    Tmod,
    TrsakeyValueType,
    Tuf,
    TufEmi,
)
from nfelib.nfe.bindings.v4_0.xmldsig_core_schema_v1_01 import (
    KeyInfoType,
    ReferenceType,
    Signature,
    SignatureType,
    SignatureValueType,
    SignedInfoType,
    TransformsType,
    TransformType,
    TtransformUri,
    X509DataType,
)

__all__ = [
    "ConsReciNfe",
    "ConsSitNfe",
    "ConsStatServ",
    "Talczfmcbs",
    "TalczfmcbsNfe",
    "TalczfmcbsNfeTpAlczfmcbs",
    "TajusteCompet",
    "Tcibs",
    "TcibsNfe",
    "TcompraGov",
    "TcompraGovReduzido",
    "TcredPres",
    "TcredPresIbszfm",
    "TcredPresOper",
    "TdevTrib",
    "Tdif",
    "TenteGov",
    "TestornoCred",
    "TibscbsmonoTot",
    "Tibscbstot",
    "Tis",
    "Tistot",
    "TindDoacao",
    "Tmonofasia",
    "ToperCompraGov",
    "TpagRef",
    "Tred",
    "TtpCredPresIbszfm",
    "TtransfCred",
    "TtribBpe",
    "TtribCte",
    "TtribCompraGov",
    "TtribNf3E",
    "TtribNfag",
    "TtribNfce",
    "TtribNfcom",
    "TtribNfgas",
    "TtribNfe",
    "TtribRegular",
    "EnviNfe",
    "InutNfe",
    "TconsSitNfe",
    "TconsSitNfeXServ",
    "Tevento",
    "TprocEvento",
    "TprotNfe",
    "TretCancNfe",
    "TretConsSitNfe",
    "TretEvento",
    "TverConsSitNfe",
    "TconsStatServ",
    "TconsStatServXServ",
    "TretConsStatServ",
    "TinutNfe",
    "TprocInutNfe",
    "TretInutNfe",
    "InfInutXServ",
    "CofinsaliqCst",
    "CofinsntCst",
    "CofinsoutrCst",
    "CofinsqtdeCst",
    "CofinsstIndSomaCofinsst",
    "DiTpIntermedio",
    "DiTpViaTransp",
    "Icms00Cst",
    "Icms00ModBc",
    "Icms02Cst",
    "Icms10Cst",
    "Icms10ModBc",
    "Icms10ModBcst",
    "Icms10MotDesIcmsst",
    "Icms15Cst",
    "Icms15MotRedAdRem",
    "Icms20Cst",
    "Icms20IndDeduzDeson",
    "Icms20ModBc",
    "Icms20MotDesIcms",
    "Icms30Cst",
    "Icms30IndDeduzDeson",
    "Icms30ModBcst",
    "Icms30MotDesIcms",
    "Icms40Cst",
    "Icms40IndDeduzDeson",
    "Icms40MotDesIcms",
    "Icms51Cst",
    "Icms51ModBc",
    "Icms53Cst",
    "Icms60Cst",
    "Icms61Cst",
    "Icms70Cst",
    "Icms70IndDeduzDeson",
    "Icms70ModBc",
    "Icms70ModBcst",
    "Icms70MotDesIcms",
    "Icms70MotDesIcmsst",
    "Icms90Cst",
    "Icms90IndDeduzDeson",
    "Icms90ModBc",
    "Icms90ModBcst",
    "Icms90MotDesIcms",
    "Icms90MotDesIcmsst",
    "IcmspartCst",
    "IcmspartIndDeduzDeson",
    "IcmspartModBc",
    "IcmspartModBcst",
    "IcmspartMotDesIcms",
    "Icmssn101Csosn",
    "Icmssn102Csosn",
    "Icmssn201Csosn",
    "Icmssn201ModBcst",
    "Icmssn202Csosn",
    "Icmssn202ModBcst",
    "Icmssn500Csosn",
    "Icmssn900Csosn",
    "Icmssn900ModBc",
    "Icmssn900ModBcst",
    "IcmsstCst",
    "IcmsufdestPIcmsinter",
    "IpintCst",
    "IpitribCst",
    "IssqnIndIss",
    "IssqnIndIncentivo",
    "IssqntotCRegTrib",
    "PisaliqCst",
    "PisntCst",
    "PisoutrCst",
    "PisqtdeCst",
    "PisstIndSomaPisst",
    "TconsReciNfe",
    "TenderEmi",
    "TenderEmiCPais",
    "TenderEmiXPais",
    "Tendereco",
    "TenviNfe",
    "TenviNfeIndSinc",
    "TfinNfe",
    "TinfRespTec",
    "Tipi",
    "Tlocal",
    "Tnfe",
    "TnfeProc",
    "TprocEmi",
    "TretConsReciNfe",
    "TretEnviNfe",
    "TtpNfcredito",
    "TtpNfdebito",
    "Tveiculo",
    "Torig",
    "ArmaTpArma",
    "CardTpIntegra",
    "DestIndIedest",
    "DetPagIndPag",
    "EmitCrt",
    "GuiaTransitoTpGuia",
    "IdeIdDest",
    "IdeIndFinal",
    "IdeIndIntermed",
    "IdeIndPres",
    "IdeTpEmis",
    "IdeTpImp",
    "IdeTpNf",
    "OrigCombIndImport",
    "ProcRefIndProc",
    "ProcRefTpAto",
    "ProdIndBemMovelUsado",
    "ProdIndEscala",
    "ProdIndTot",
    "RefEcfMod",
    "RefNfpMod",
    "RefNfMod",
    "TranspModFrete",
    "VeicProdVin",
    "VeicProdCondVeic",
    "VeicProdTpOp",
    "VeicProdTpRest",
    "Nfe",
    "ProcInutNfe",
    "NfeProc",
    "RetConsReciNfe",
    "RetConsSitNfe",
    "RetConsStatServ",
    "RetEnviNfe",
    "RetInutNfe",
    "Tamb",
    "TcorgaoIbge",
    "TcodUfIbge",
    "Tmod",
    "TrsakeyValueType",
    "Tuf",
    "TufEmi",
    "KeyInfoType",
    "ReferenceType",
    "Signature",
    "SignatureType",
    "SignatureValueType",
    "SignedInfoType",
    "TtransformUri",
    "TransformType",
    "TransformsType",
    "X509DataType",
]
