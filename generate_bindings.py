...
CHANGELOG_PATH: Final = ROOT / "CHANGELOG_SCHEMAS.md"


def _load_schema_metadata() -> dict[str, dict[str, str]]:
    """Parse CHANGELOG_SCHEMAS.md and return metadata keyed by binding_dir."""
    ...
    for section in sections[1:]:
        binding_dir = re.search(r"binding_dir:\s*`([^`]+)`", section)
        if not binding_dir:
            continue
        key = binding_dir.group(1)
        entry: dict[str, str] = {}
        for field in (
            "version",
            "package",
...
SCHEMA_METADATA: Final[dict[str, dict[str, str]]] = _load_schema_metadata()


def _binding_dir_from_package(package: str) -> str:
    """Derive the relative binding directory from an xsdata package name.

    e.g. ``nfelib.nfe.bindings.v4_0`` -> ``nfe/bindings/v4_0``
    """
    return "/".join(package.split(".")[1:])


def _write_init_metadata(package: str) -> None:
    """Write an __init__.py with schema metadata for the generated package."""
    binding_dir = _binding_dir_from_package(package)
    meta = SCHEMA_METADATA.get(binding_dir)
    if not meta:
        print(f"  no metadata found in CHANGELOG_SCHEMAS.md for {binding_dir}")
        return

    package_path = ROOT / package.replace(".", "/")
    init_file = package_path / "__init__.py"

    # When xsdata is configured with StructureStyle.SINGLE_PACKAGE, the target
    # can be a module (e.g. ``nfe_epec.bindings.v1_0.e110140_v1_00``). In that
    # case xsdata creates the parent package's __init__.py and then writes a
    # sibling module. We only update an existing __init__.py if it exists.
    ...
    header = f'''"""Auto-generated schema metadata.

This module is part of the generated binding package `{package}`.
The source XSD schema package details are tracked below.
"""

# Source schema package information (from CHANGELOG_SCHEMAS.md)
SCHEMA_VERSION: str = {meta.get("version", "")!r}
SCHEMA_PACKAGE: str = {meta.get("package", "")!r}
NOTA_TECNICA: str = {meta.get("nota_tecnica", "")!r}
PUBLISHED_AT: str = {meta.get("published_at", "")!r}
SOURCE_URL: str = {meta.get("source_url", "")!r}
GENERATED_AT: str = {generated_at!r}

'''
    # Avoid duplicating metadata blocks on repeated runs.
    if "SCHEMA_VERSION" in content:
        original_start = content.find('"""This file was generated by xsdata')
        if original_start == -1:
            original_start = content.find('"""', 3)
        if original_start != -1:
            content = content[original_start:]

    init_file.write_text(header + content, encoding="utf-8")
    print(f"  updated metadata in {init_file}")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do not generate bindings; only inject schema metadata into the "
            "existing __init__.py files. Useful after updating CHANGELOG_SCHEMAS.md."
        ),
    )
